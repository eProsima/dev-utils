        ...
        std::function<void(Args...)> callback) noexcept
{
    ...
    {
        logDebug(UTILS_EVENTHANDLER, "Setting new callback to an EventHandler that already had one.");
...
void EventHandler<Args...>::unset_callback() noexcept
{
    ...
    {
        logWarning(UTILS_HANDLER, "Unsetting callback from an EventHandler that had no callback set.")
    }
        ...
        Args... args) noexcept
{
    ...
    {
        logInfo(UTILS_HANDLER, "Skipping callback in a not enabled EventHandler.");
    }
