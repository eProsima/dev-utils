...
void WaitHandler<T>::enable() noexcept
{
    ...
    if (!enabled_.load())
    {
        ...
        logDebug(UTILS_WAIT, "Enabling WaitHandler.");
        enabled_.store(true);
    }
    else
    {
        logDebug(UTILS_WAIT, "Enabling already enabled WaitHandler.");
    }
}
...
void WaitHandler<T>::disable() noexcept
{
    ...
    {
        {
            // WARNING: enabled_ should be modified with mutex taken
            std::lock_guard<std::mutex> lock(wait_condition_variable_mutex_);
            logDebug(UTILS_WAIT, "Disabling WaitHandler.");
    ...
    {
        logDebug(UTILS_WAIT, "Disabling already disabled WaitHandler.");
    }
}
