...
{
    // Create thread_pool
    threading::StdThreadPool thread_pool(n_threads, false);
    thread_pool.start();
    ...
    utils::event::IntWaitHandler waiter(0);
    ...
    utils::event::IntWaitHandlerType target_value = utils::arithmetic_progression_sum(1, 1, m_tasks);
