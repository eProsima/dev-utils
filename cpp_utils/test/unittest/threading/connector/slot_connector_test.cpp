...
TEST(SlotConnectorTest, slot_test_no_params)
{
    ...
    // Create slot by copy
    utils::threading::SimpleSlotConnector once_slot(manager, lambda);
    ...
    utils::threading::SimpleSlotConnector move_slot(manager, std::move(lambda));
    ...
    // Wait for lambda to be called required times
    utils::event::IntWaitHandlerType target_value = test::DEFAULT_TIME_REPETITIONS;
...
TEST(SlotConnectorTest, slot_test_int)
{
    ...
    // Create slot by copy
    utils::threading::SlotConnector<int> once_slot(manager, lambda);
    ...
    // Execute lambda N times by moving increasing in 1 by moving
    utils::threading::SlotConnector<int> move_slot(manager, std::move(lambda));
    for (unsigned int i = 1; i <= test::DEFAULT_TIME_REPETITIONS; ++i)
    {
        move_slot.execute(i);
    }
    ...
    // Wait for lambda to be called required times
    utils::event::IntWaitHandlerType target_value = utils::arithmetic_progression_sum(1, 1, test::DEFAULT_TIME_REPETITIONS);
    counter.wait_equal(target_value);
    // Check that lambda has been called only that amount of times
    ASSERT_EQ(counter.get_value(), target_value);
...
TEST(SlotConnectorTest, slot_test_string)
{
    ...
    // Create slot by copy
    utils::threading::SlotConnector<std::string> once_slot(manager, lambda);
    ...
    // Execute lambda N times by moving increasing in 1 by moving
    utils::threading::SlotConnector<std::string> move_slot(manager, std::move(lambda));
    for (unsigned int i = 1; i <= test::DEFAULT_TIME_REPETITIONS; ++i)
    {
...
TEST(SlotConnectorTest, slot_test_bool_int_string)
{
    ...
    // Create slot by copy
    utils::threading::SlotConnector<bool, int, std::string> once_slot(manager, lambda);
    ...
    utils::threading::SlotConnector<bool, int, std::string> move_slot(manager, std::move(lambda));
    for (unsigned int i = 1; i <= test::DEFAULT_TIME_REPETITIONS; ++i)
    {
...
TEST(SlotConnectorTest, slot_test_complex_args)
{
    ...
    utils::threading::SlotConnector<utils::event::IntWaitHandler&, const int> move_slot(
        manager,
